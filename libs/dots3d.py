...
class Tools:
	...
	def get_pic_from_file(self, path):
		"""Reads ply file and extracts its data.
		"""
	...
	def get_next_pnts(self, curPnt, short_set=True):
		"""returns 8 points around curPnt
		"""
		nextPnts = []
...
class Object(Tools):
	def __init__(self, points, heights, img, color=[0, 0, 0]) -> None:
		...
		self.centerz = sum(self.heights.values()) / len([i for i in heights if i != 0]) # medium between all the heights of the surface
		#default y, x, z
		self.globaly=-0.20
		self.globalx=-0.80
		self.globalz=0.65
		...
		# get rotation angle
		self.box = [[y, x] for x, y in list(self.box)] # to (y, x)
		sorted_corners = self.sort_clockwise(self.box, [self.centery, self.centerx])
		...
		if (a > b):
			# tilted to the left
			self.angle = acos((sorted_corners[1][1] - y_low_coord[1]) / a)
		else:
			# tilted to the right
			self.angle = acos((sorted_corners[3][1] - y_low_coord[1]) / b)
	...
	def check_around(self, trinarised_img):
		img = np.array(trinarised_img, dtype=np.uint8)

		box = np.array(self.box)
		A, B, C, D = box
		center = np.array([self.centery, self.centerx])
		...
		img = self.rotate_image(img, -degrees(self.angle))
		...
		O = np.int0(np.round(sum(box) / 4))
		O = [O[1], O[0]]
		box_list = self.sort_clockwise([(y, x) for x, y in box], O)

		y_low_coord = min(box_list, key=lambda x: self.dist(x, (0, 0)))
	...
	def get_angle(self, trinarised_img):
		result = self.check_around(trinarised_img)
		
		if (result == 1):
			return (True, pi / 2 - self.angle, 1)
		elif (result == 2):
			if (self.angle >= pi / 2):
				return (True, pi - self.angle, 2)
			else:
				return (True, -self.angle, 2)
		return (False, pi / 2 - self.angle, 1)
...
class MatrixProcessing(Tools):
	...
	def determine_blocks(self, image):
		hgts = [[[0, 255-self.mt_height[y][x], 0] if self.mt_color[y][x]!=[0,0,0] else [0,0,0] for x in range(self.WIDTH)] for y in range(self.HEIGHT)]
	...
	def create_objects(self):
		obj = []
		blocks = self.determine_blocks(self.mt_color)
		for block in blocks.values():
			if len(block) < 370 : continue
			h = [self.mt_height[p[0]][p[1]] for p in block]
			img = [[[0, 0, 0] for _ in line] for line in self.mt_color]
